from os import sched_get_priority_max
import pygame
import random

# creating the data structure for pieces
# setting up global vars
# functions
# - create_grid
# - draw_grid
# - draw_window
# - rotating shape in main
# - setting up the main

"""
10 x 20 square grid
shapes: S, Z, I, O, J, L, T
represented in order by 0 - 6
"""

pygame.font.init()

# GLOBALS VARS
s_width = 800
s_height = 700
play_width = 300  # meaning 300 // 10 = 30 width per block
play_height = 600  # meaning 600 // 20 = 20 height per block
block_size = 30

top_left_x = (s_width - play_width) // 2
top_left_y = s_height - play_height


# SHAPE FORMATS

S = [['.....',
      '......',
      '..00..',
      '.00...',
      '.....'],
     ['.....',
      '..0..',
      '..00.',
      '...0.',
      '.....']]

Z = [['.....',
      '.....',
      '.00..',
      '..00.',
      '.....'],
     ['.....',
      '..0..',
      '.00..',
      '.0...',
      '.....']]

I = [['..0..',
      '..0..',
      '..0..',
      '..0..',
      '.....'],
     ['.....',
      '0000.',
      '.....',
      '.....',
      '.....']]

O = [['.....',
      '.....',
      '.00..',
      '.00..',
      '.....']]

J = [['.....',
      '.0...',
      '.000.',
      '.....',
      '.....'],
     ['.....',
      '..00.',
      '..0..',
      '..0..',
      '.....'],
     ['.....',
      '.....',
      '.000.',
      '...0.',
      '.....'],
     ['.....',
      '..0..',
      '..0..',
      '.00..',
      '.....']]

L = [['.....',
      '...0.',
      '.000.',
      '.....',
      '.....'],
     ['.....',
      '..0..',
      '..0..',
      '..00.',
      '.....'],
     ['.....',
      '.....',
      '.000.',
      '.0...',
      '.....'],
     ['.....',
      '.00..',
      '..0..',
      '..0..',
      '.....']]

T = [['.....',
      '..0..',
      '.000.',
      '.....',
      '.....'],
     ['.....',
      '..0..',
      '..00.',
      '..0..',
      '.....'],
     ['.....',
      '.....',
      '.000.',
      '..0..',
      '.....'],
     ['.....',
      '..0..',
      '.00..',
      '..0..',
      '.....']]

shapes = [S, Z, I, O, J, L, T]
shape_colors = [(0, 255, 0), (255, 0, 0), (0, 255, 255), (255, 255, 0), (255, 165, 0), (0, 0, 255), (128, 0, 128)]

#
class Piece(object):
    def __init__(self, x, y, shape):
        self.x = x
        self.y = y
        self.shape = shape
        self.color = shape_colors[ shapes.index(shape) ]
        self.rotation = 0

# create main grid for tetris
def create_grid( locked_pos={} ):
    grid = [ [ (0,0,0) for x in range(10)  ] for x in range(20) ]

    # ( x, y ) : ( 0, 0, 255 )
    for y in range( len(grid) ):
        for x in range( len( grid[y]) ):
            if ( x,y ) in locked_pos:
                grid[y][x] = locked_pos[ (x,y) ]

    return grid

#
def convert_shape_format(shape):
    position = []
    format = shape.shape[ shape.rotation % len(shape.shape) ]

    for y, line in enumerate(format):
        for x, each in enumerate(line):
            if each == '0':
                position.append( ( shape.x + x,  shape.y + y ) )
    ###
    for i, pos in enumerate(position):
        position[i] = (pos[0] - 2, pos[1] - 4)

    return position



#
def valid_space(shape, grid):
    accepted_pos = [ [(x, y) for x in range(10) if grid[y][x] == (0,0,0)] for y in range(20) ]
    accepted_pos = [ j for sub in accepted_pos for j in sub ]

    format = convert_shape_format(shape)

    for pos in format:
        if pos not in accepted_pos:
            ###
            if pos[1] > -1:
                return False
    return True


#
def check_lost(positions):
    for pos in positions:
        x, y = pos
        if y < 1:
            return True

    return False

#
def get_shape():
    return Piece(5, 0, random.choice(shapes))

def draw_text_middle(surface, text, size, color):
    pass

#
def draw_grid(surface, grid):

    for i in range( len( grid ) ):
        pygame.draw.line( surface, (255,255,255), ( top_left_x, top_left_y + i*block_size ), ( top_left_x + play_width, top_left_y + i*block_size ), 2 )
        for j in range( len( grid[i] ) ):
            pygame.draw.line( surface, (255,255,255), ( top_left_x + j*block_size , top_left_y ), ( top_left_x + j*block_size, top_left_y + play_height ), 2 )


def clear_rows(grid, locked):
    inc = 0
    for y in range(len(grid)-1, -1, -1):
        row = grid[y]
        if (0,0,0) not in row:
            inc += 1
            ind = y
            for x in range(len(row)):
                del locked[ ( x, y ) ]

    if inc > 0:
        for key in sorted(list(locked), key=lambda x: x[1])[::-1]:
            x, y = key
            if y < ind:
                newKey = ( x, y + inc)
                locked[ newKey ] = locked.pop(key)







def draw_next_shape(shape, surface):
    pass

#
def draw_window(surface, grid):
    surface.fill((0, 0, 0))
    pygame.font.init()
    font = pygame.font.SysFont('comicsans', 60)
    label = font.render( "Tetris Girls", 1, (255,0,0) )

    surface.blit( label, ( s_width/2  - ( label.get_width()/2 ), 30 ) )

    for y in range(len(grid)):
        for x in range(len(grid[y])):
            pygame.draw.rect(surface, grid[y][x], (top_left_x + x*block_size, top_left_y + y*block_size, block_size, block_size), 0)

    pygame.draw.rect( surface, (0,0,255), ( top_left_x, top_left_y, play_width, play_height), 5 )

    draw_grid( surface, grid )

#
def main( win ):
    locked_positions = {}
    grid = create_grid(locked_positions)

    # check if piece is positioned
    change_piece = False

    # boolean for piece movement in grid
    run = True

    #First Tetris Piece
    current_piece = get_shape()
    #Next Tetris Piece
    next_piece = get_shape()

    clock = pygame.time.Clock()
    fall_time = 0
    fall_speed = 0.27

    while run:
        grid = create_grid(locked_positions)
        fall_time += clock.get_rawtime()
        clock.tick()

        # reset fall_time to 0 and drop tetris piece if fall_time goes over fall_speed threshold
        if fall_time/1000 > fall_speed:
            fall_time = 0
            current_piece.y += 1

            # find if next piece movement is not floor or other piece
            if not(valid_space(current_piece, grid)) and current_piece.y > 0:
                current_piece.y -= 1
                change_piece = True

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
                pygame.display.quit()

            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    current_piece.x -= 1
                    if not(valid_space(current_piece, grid)):
                        current_piece.x += 1
                if event.key == pygame.K_RIGHT:
                    current_piece.x += 1
                    if not(valid_space(current_piece, grid)):
                        current_piece.x -= 1
                if event.key == pygame.K_DOWN:
                    current_piece.y += 1
                    if not(valid_space(current_piece, grid)):
                        current_piece.y -= 1
                if event.key == pygame.K_UP:
                    current_piece.rotation += 1
                    if not(valid_space(current_piece, grid)):
                        current_piece.rotation -= 1



        # gets coordinate of tetris piece position
        shape_pos = convert_shape_format(current_piece)

        # puts color into tetris position
        for i in range(len(shape_pos)):
            x, y = shape_pos[i]
            if y > -1:
                grid[y][x] = current_piece.color

        # if change_piece is true(dropped to the bottom) save position and show next piece
        # (x,y) : (255, 255, 255)
        if change_piece:
            for pos in shape_pos:
                p = (pos[0], pos[1])
                locked_positions[p] = current_piece.color
            current_piece = next_piece
            next_piece = get_shape()
            change_piece = False
            clear_rows(grid, locked_positions)
       
        draw_window(win, grid)
        
        pygame.display.update()


        if check_lost(locked_positions):
            run = False


def main_menu( win ):  # start game
    run = True

    while run:
        win.fill((0,0,0))
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            if event.type == pygame.KEYDOWN:
                main(win)

    pygame.display.quit()


win = pygame.display.set_mode((s_width, s_height))
pygame.display.set_caption('Tetris')
main_menu(win)